...
var courses = ["node js", "PHP", "Java core", "HTML"]; //map method 
...
var ht = courses.map3(function (value, index) {
  return value;
});
console.log("(MAP) Aray's Element: ", ht); //forEach
...
var SUM = coursePrice.reduce2(function (start_value, element, index, all) {
  return start_value + element;
}, 0);
console.log("Sum by reduce: ", SUM); // EVERY & FIND & FILLTER
...
var valueOfCheck = Dog.every2(function (Element) {
  return Element.age === 19;
});
console.log("all member is 19? ", valueOfCheck); // //some2: trả về true nếu có ít nhất 1 phần tử thỏa mãn điều kiện: đã test nhưng k biết có đúng k
...
var checkSome = Dog.some2(function (element, index) {
  return element.age === 14;
});
console.log("the Element satisfies the condition: ", checkSome); // find2: trả về phần tử đầu tiên thỏa mãn điều kiện, các phần tử phía sau cho dù thỏa mãn nhưng sẽ bị bỏ qua: done, đã oke
...
console.log("Find: ", check); //fillter: trả về tất cả các phần tử thỏa mãn điều kiện
...
Array.prototype.fillter2 = function (callBack) {
  ...
  for (var i = 0; i < lenArray; i++) {
    if (callBack(this[i], i)) arrayTemp.push(this[i]);
  } // return arrayTemp;
